...
export default function Main() {
  ...
  // const samplePills: Record<string, string[]> = {
  //   "2025-11-10": ["아침: 타이레놀 1정", "점심: 비타민C 1정"],
  //   "2025-11-11": ["아침: 타이레놀 2정"],
  // };

  // const getTodayPills = (date: Date | null) => {
  //   if (!date) return [];
  //   const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(
  //     2,
  //     "0"
  //   )}-${String(date.getDate()).padStart(2, "0")}`;
  //   return samplePills[key] || [];
  // };
  ...
  // useEffect(() => {
  //   const saved = localStorage.getItem("drugPeriod");
  //   if (saved) {
  ...
  const getTodayPills = (date: Date | null): TodayPillItem[] => {
    ...
    return drugSchedules
      .filter((schedule) => {
        ...
        const isInPeriod = current >= start && current <= end;

        const scheduledDays = schedule.days.split(",").map((d) => d.trim());
        return isInPeriod && scheduledDays.includes(todayDayInKorean);
      })
